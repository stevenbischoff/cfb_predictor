...
class NeuralNet():
  def __init__(self, n, week, window = 2, learn_rate = 0.0001, season_discount = 0, tol = 0.0001): 
  ...
  def assess(self, learn_rate_counter, threshold): 
    ...
    """
    if self.test_error > (self.best_test_error - self.tol) and self.n_worse >= 2 and\
...
def ratings_calculation(nn, game_data):
  ...
  np.apply_along_axis(team_rating, 1, game_data1, nn, game_data, col_cutoff, last_season)


def team_rating(tg, nn, game_data, col_cutoff, last_season):
  """
    ...
    for season in range(first_season, last_season + 1):
      try:
        if cfg.ratings_dict[team][str(season)+'League'] != 'FCS':
          ...
          a = np.array([cfg.ratings_dict[opp][str(season) + 'Rating'] for opp in opps])
          avg_sos = pd.Series(np.concatenate([[0.5], np.cumsum(a)/np.arange(1, len(a) + 1)]),
...
def ratings_sos_calculation(nn, game_data, first_season, last_season, n_rounds):
  """
  Performs a specified number of ratings and SOS updates.
  """
  for n in range(n_rounds):
    ratings_calculation(nn, game_data)        
    sos_calculation(game_data, first_season, last_season)
